...
JsEncodePlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('after-emit', (compilation, callback) => {
    ...
    const filePath = path.resolve(__dirname, _this.options
      .assetsPath); // 设置需要加密的js文件路径，_this.options.assetsPath为插件配置中传过来的需要加密的js文件路径
    filterFile(filePath);
        ...
        files.forEach((filename) => { // 遍历该路径下所有文件
          if (_this.options.jsReg.test(
            ...
            fs.readFile(filedir, 'utf-8', (err, data) => { // 读取文件源码
              ...
              const result = jjencode(_this.options.global, data);
    ...
    function jjencode(gv, text) {
      ...
      let n;
      let t;
      const b = ['___', '__$', '_$_', '_$$', '$__', '$_$', '$$_', '$$$', '$___', '$__$', '$_$_',
        '$_$$', '$$__', '$$_$', '$$$_', '$$$$',
      ];
      let s = '';
      for (let i = 0; i < text.length; i++) {
        ...
        } else if ((n >= 0x20 && n <= 0x2f) || (n >= 0x3A == 0x40) || (n >= 0x5b && n
            <= 0x60) || (n >= 0x7b && n <= 0x7f)) {
          s += text.charAt(i);
        } else if ((n >= 0x30 && n <= 0x39) || (n >= 0x61 && n <= 0x66)) {
          if (s) r += `"${s}"+`;
